...
//    echo $GLOBALS['_LANG']['query_info']."<br>";
//    echo $GLOBALS['db']->queryCount."<br>";
//    echo $query_time."<br>";
...
function load_config()
{
    ...
    if ($data === false)
    {
        ...
        {
        	if($row['type'] == 'int'){
            	$arr[$row['code']] = intval($row['value']);
        	}elseif ($row['type'] == 'float') {
        		$arr[$row['code']] = floatval($row['value']);
        	}else {
        ...
        if (!isset($GLOBALS['_CFG']['ddt_version']))
        {
            /* 如果没有版本号则默认为2.0.5 */
            $GLOBALS['_CFG']['ddt_version'] = 'v1.0.0';
        }
        ...
        {
            $arr['integrate_code'] = 'diandiantao'; // 默认的会员整合插件为 ecshop
        }
        write_static_cache('ddt_config', $arr);
    }
    else
    {
        $arr = $data;
