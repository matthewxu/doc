...
function send_mail($name, $email, $subject, $content, $type = 0, $notification=false)
{
    ...
    {
        $name      = ddt_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $name);
        $subject   = ddt_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $subject);
        $content   = ddt_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $content);
        $GLOBALS['_CFG']['shop_name'] = ddt_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $GLOBALS['_CFG']['shop_name']);
    }
 ...
 * @return  void
 **/
function ddt_header($string, $replace = true, $http_response_code = 0)
{
...
function ddt_iconv($source_lang, $target_lang, $source_string = '')
{
    static $chs = NULL;
...
function json_str_iconv($str)
{
    if (EC_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return ddt_iconv('utf-8', EC_CHARSET, $str);
        }
        elseif (is_array($str))
        {
...
function to_utf8_iconv($str)
{
    if (EC_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return ddt_iconv(EC_CHARSET, 'utf-8', $str);
        }
        elseif (is_array($str))
        {
            foreach ($str as $key => $value)
