...
function get_profile($user_id)
{
    ...
    $info  = array();
    $infos = array();
    $sql  = "SELECT username, birthday, sex, question, answer,".
             " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer ".
           "FROM " .$GLOBALS['ddt']->table('user') . " WHERE id = '$user_id'";
    $infos = $GLOBALS['db']->getRow($sql);
    $infos['username'] = addslashes($infos['username']);

    $row = $user->get_profile_by_name($infos['username']); //获取用户帐号信息
    ...
    $info['question'] = $infos['question'];
    $info['answer'] = $infos['answer'];
