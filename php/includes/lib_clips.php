 ...
 * @return  array   $arr
 */
function get_history($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $history[$row['id']]['id']        = $row['id'];
        $history[$row['id']]['user_id']   = $row['user_id'];
        $history[$row['id']]['url']  	  = $row['url'];
        $history[$row['id']]['viewdate']      = $row['viewdate'];
        $history[$row['id']]['metacontent']      = $row['metacontent'];
    }
