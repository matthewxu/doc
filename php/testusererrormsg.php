...
$user_login_arr=array('act_edit_password','password','add_tag');

/* 显示页面的action列表 */
$ui_arr = array('recovery', 'password', 'login', 'signin', 'tag', 'collect', 'history',
'clear_history','logout');

/* 未登录处理 */
if (empty($_SESSION['user_id']))
{
    if (in_array($action, $user_login_arr))
    {
...
{
    assign_template();
    $smarty->assign('title', $position['title']); // 页面标题
...
//-- 活动列表页
/*------------------------------------------------------ */
//require(ROOT_PATH.'adminm/' . 'includes/lib_goods.php');
//$exc = new exchange($ecs->table('favourable_activity'), $db, 'act_id', 'act_name');
elseif ($action == 'favour_list')
{
